...
class Mail extends Model
{
    ...
    {
        $allMessages =  Mail::select('profile_infos.profile', 'users.username', 'mail_table.*','mail_subjects.message','mail_subjects.message_subject')
          ->join('users', 'users.id', '=', 'mail_table.to_id')
          ->join('profile_infos', 'profile_infos.user_id', '=', 'mail_table.to_id')
          ->join('mail_subjects', 'mail_subjects.id', 'mail_table.message')
          ->whereNull('mail_table.deleted_at')->where('mail_table.from_id', [$id])->orderBy('mail_table.created_at', 'desc')->simplepaginate(10, ['*'], 'outbox');
        ;
        $message_count = count($allMessages);
      // dd($allMessages);
        for ($i = 0; $i < $message_count; $i++) {
            $from_id = User::userIdToName($allMessages[$i]->from_id);
            $allMessages[$i]->from_id = ($from_id == 'SHEHEME001')? 'Customer Support' : $from_id;
            $allMessages[$i]->username = ($allMessages[$i]->username == 'SHEHEME001')? 'Customer Support' : $allMessages[$i]->username;
            $allMessages[$i]->full_message = substr($allMessages[$i]->message, 0, 30);
        }
        //dd($allMessages);
        return $allMessages;
    ...
    {
        $messages = Mail::select('profile_infos.profile', 'users.username', 'users.email', 'mail_table.*','mail_subjects.message','mail_subjects.message_subject')
          ->join('users', 'users.id', '=', 'mail_table.from_id')
          ->join('profile_infos', 'profile_infos.user_id', '=', 'mail_table.from_id')
          ->join('mail_subjects', 'mail_subjects.id', 'mail_table.message')
          ->whereNull('mail_table.deleted_at')->where('mail_table.to_id', [$user_id])->orderBy('created_at', 'desc')
          ->simplepaginate(10, ['*'], 'inbox');
        ...
        for ($i = 0; $i < $message_count; $i++) {
            $from_id = User::userIdToName($messages[$i]->from_id);
            $messages[$i]->from_id = ($from_id == 'SHEHEME001')? 'Customer Support' : $from_id;
            $messages[$i]->username = ($messages[$i]->username == 'SHEHEME001')? 'Customer Support' : $messages[$i]->username;
    ...
    {
        $unread_messages = DB::table('mail_table')
        ->join('users', 'users.id', '=', 'mail_table.from_id')
        ->whereNull('mail_table.deleted_at')
        ->where('mail_table.read', 'no')
        ->where('mail_table.to_id', [$user_id])
        ->select('mail_table.*', 'users.username')->get();
        // foreach ($unread_messages as $key => $value) {
        //     if($value->username == 'SHEHEME001')
        //         $value->username = 'Customer Support';
        // }
