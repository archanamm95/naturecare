...
use App\Models\Helpdesk\Ticket\TicketReply;
use App\TicketTags;
use App\User;
use Auth;
use DB;
...
class TicketsController extends AdminController
{
    public function dashboard()
    {
        ...
        return view(
            'app.admin.helpdesk.tickets.dashboard',
            compact(
                'title',
                'sub_title',
                'base',
    ...
    public function index(Request $request)
    {
        ...
        $method    = trans('ticket_details.tickets');
           ...
           // $ticket_types = TicketType::all();
              $ticket_types = TicketType::where('ispublic', 1)->get();
...
// dd($ticket_replies);
    ...
    //
        ...
        $title     = trans('ticket.view_ticket');
        $sub_title = trans('ticket.view_ticket');
        $base      = trans('ticket.view_ticket');
        $method    = trans('ticket.view_ticket');
   ...
   /**
    ...
    public function update($id, TicketRequest $request)
    {
        ...
        try {
            ...
            $ticket->status = $request->status;
            $ticket->type = $request->type;
            // $ticket->ticket_number =  $request->type;
            // $ticket->user_id = $request->user_id;
            $ticket->ip_address = $request->ip_address;
            $ticket->last_message_at = $request->last_message_at;
            if ($ticket->save()) {
    ...
    public function destroy($id)
    {
        ...
        $ticket->delete();
        Session::flash('flash_notification', array('level' => 'success', 'message' => trans('ticket.ticket_deleted_successfully')));
        return redirect('admin/helpdesk/ticket');
    }
    ...
    public function data(Request $request)
    {
        ...
        try {
            ...
            // $tickets = Ticket::join('users','users.id','tickets.user_id')->join('ticket_departments','ticket_departments.id','tickets.department')->select('tickets.id','ticket_number','users.name','subject','status','priority','ticket_departments.name as department','category');
            if ($request->ticketnum <> 'null') {
                $user_id=Ticket::where('ticket_number', $request->ticketnum)->value('id');
                if ($user_id == null) {
                     $tickets = Ticket::join('users', 'users.id', 'tickets.user_id')
                     ->where('tickets.ticket_number', 0)
                         ->select('tickets.id', 'ticket_number', 'users.name', 'subject', 'tickets.status', 'priority', 'department', 'category');
                } else {
                    $tickets = Ticket::join('users', 'users.id', 'tickets.user_id')
                     ->where('tickets.ticket_number', $request->ticketnum)
                         ->select('tickets.id', 'ticket_number', 'users.name', 'subject', 'tickets.status', 'priority', 'department', 'category');
                }
            } elseif ($request->userkeyword <> 'null') {
                $ticketuser_id=User::where('username', $request->username)->value('id');
                if ($ticketuser_id <> 'null') {
                    $tickets = Ticket::join('users', 'users.id', 'tickets.user_id')
                     ->where('tickets.user_id', $ticketuser_id)
                         ->select('tickets.id', 'ticket_number', 'users.name', 'subject', 'tickets.status', 'priority', 'department', 'category');
                } else {
                    $tickets = Ticket::join('users', 'users.id', 'tickets.user_id')
                     ->where('tickets.user_id', 0)
                         ->select('tickets.id', 'ticket_number', 'users.name', 'subject', 'tickets.status', 'priority', 'department', 'category');
                }
            } else {
                ...
                if ($request->input('overdue') == 'on') {
                     ...
                     $tickets->whereDate('tickets.created_at', '<', $expDate);
                }
            ...
            return DataTables::of($var)
                ...
                ->addColumn('action', function ($model) {
                    return '<div style="display: flex;" class="btn-group"> <a href="'.url('admin/helpdesk/ticket/'.$model->id.'/show').'" class="btn btn-success btn-info btn-xs btn-view-ticket" data-id="'.$model->id.'"> View </a> &nbsp; <a href="'.url('admin/helpdesk/ticket/'.$model->id).'/edit" class="btn btn-success btn-info btn-xs btn-edit-ticket" data-id="'.$model->id.'"> edit </a>
                                    &nbsp;
                                    <button class="btn btn-warning btn-info btn-xs btn-delete-ticket" data-id="'.$model->id.'"> delete </button></div>';
                })
                ->escapeColumns([])
    ...
    public function getStatusMarkup($model)
    {
        ...
        if ($selected_status == 'Unverified Status') {
            $buttoncolor = 'btn-danger';
        }
        ...
        foreach ($statuses_list as $status) {
            ...
            $statuses_listmenu = $statuses_listmenu.'<a class="dropdown-item changestatusdropdownitem" data-status="'.$status->name.'" data-statusid="'.$status->id.'" data-statuscolor="'.$status->color.'" data-ticketid="'.$model->id.'" href="javascript:void(0)">'.$icontext.' '.$status->name.'</a>';
        };
  ...
  //   }
    ...
    public function viewpage($id)
    {
        ...
        $ticket_priorities = TicketPriority::where('id', $ticket->priority)->value('priority');

        $ticket_departments = TicketDepartment::where('id', $ticket->department)->value('name');
        $ticket_categories = TicketCategory::where('id', $ticket->category)->value('category');
        $ticket_statuses = TicketStatus::where('id', $ticket->status)->value('name');
        $ticket_types = TicketType::all();
        $ticketdescription = strip_tags($ticket->description);
        // $tickets = $ticket->pluck('id','subject','description','priority','type','department','category','status')->toArray();
        $title     ="View Tickets";
        $sub_title = "View Tickets";
        $base      = "View Tickets";
        $method    = "View Tickets";
        
        try {
            return view('app.admin.helpdesk.tickets.tickets.view', compact('title', 'sub_title', 'base', 'method', 'ticket', 'ticket_priorities', 'ticket_departments', 'ticket_categories', 'ticket_statuses', 'ticket_types', 'ticketdescription'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
