...
namespace App\Http\Controllers\Admin\Helpdesk\kb;
...
class ArticleController extends AdminController
{
    ...
    public function index()
    {
        ...
        /* show article list */
        try {
             return view('app.admin.helpdesk.kb.article.index', compact('title', 'sub_title', 'base', 'method', 'category'));
        } catch (Exception $e) {
    ...
    public function update($slug, ArticleUpdate $request)
    {
        ...
        $relation = new Relationship();
        $aid = $article->where('id', $slug)->first();
        $publishTime = $request->year.'-'.$request->month.'-'.$request->day.' '.$request->hour.':'.$request->minute.':00';
        $id = $aid->id;
        ...
        try {
            ...
            $article->slug = $slug;
            Article::where('id', $slug)->update(['publish_time'=>$publishTime]);
            $article->save();
    ...
    public function view($slug)
    {
        ...
        /* send to the edit page */
        try {
            return view('app.admin.helpdesk.kb.article.view', compact('assign', 'article', 'category', 'title', 'sub_title', 'base', 'method', 'description'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
    public function show($slug)
    {
        ...
        $description = strip_tags($aid->description);
        try {
            return view('app.admin.helpdesk.kb.article.show', compact('assign', 'article', 'category', 'title', 'sub_title', 'base', 'method', 'aid', 'description'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
