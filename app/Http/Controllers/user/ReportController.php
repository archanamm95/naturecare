...
class ReportController extends UserAdminController
{
    ...
    public function ewalletreport()
    {
        ...
        $base = trans('report.income_report');
        $method = trans('report.income_report');
        $reportdata=Commission::where('commission.created_at', '>', date('Y-m-d 00:00:00', strtotime(date('m/01/Y'))))
        ->where('commission.created_at', '<', date('Y-m-d 23:59:59', strtotime(date('m/t/Y'))))
        ...
        ->join('users as plcmnt','plcmnt.id','tree_table.placement_id')
        ->select('users.username', 'users.lastname', 'users.name', 'commission.created_at', 'commission.payable_amount', 'commission.payment_type', 'from.username as fromuser','plcmnt.username as placement')->orderby('created_at', 'ASC');
        ...
        $start = date('m/01/Y');
        $end = date('m/t/Y');
        return view('app.user.report.ewalletreport', compact('title', 'user', 'users', 'sub_title', 'base', 'method', 'packages', 'reportdata', 'start', 'end'));
    }
    ...
    public function ewalletreportview(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $title = trans('report.income_report');
            $sub_title = trans('users/report.income_report');
           ...
           return view('app.user.report.ewalletreportview', compact('title', 'reportdata', 'sub_title', 'base', 'method', 'start', 'end'));
    }
