...
// Requests
use App\Http\Requests\Helpdesk\kb\ArticleRequest;
use App\Http\Requests\Helpdesk\kb\ArticleUpdate;
// Models
use App\Models\Helpdesk\kb\Article;
use App\Models\Helpdesk\kb\Category;
use App\Http\Requests\Helpdesk\kb\CategoryRequest;
use App\Http\Requests\Helpdesk\kb\CategoryUpdate;
...
class Knowledgebase extends UserAdminController
{
    ...
    //                     /* add column action */
        ->addColumn('action', function ($model) {
                    return ' <a class="btn btn-xs btn-success" href="'.url('user/helpdesk/kb/article/'.$model->id.'/view/').'/">View</a>&nbsp;
                    <a class="btn btn-xs btn-success" href="'.url('user/helpdesk/kb/article/'.$model->id.'/edit/').'/">Edit</a>
                                    &nbsp;
                                    <button class="btn btn-danger btn-info btn-xs btn-delete-kb-article2" data-id="'.$model->slug.'"> Delete </button>';
        })
    ...
    public function edit($slug)
    {
        ...
        try {
            return view('app.user.helpdesk.kb.article.edit', compact('assign', 'article', 'category', 'title', 'sub_title', 'base', 'method'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
    ...
    public function destroy($slug, Article $article, Relationship $relation, Comment $comment)
    {
        ...
        if ($article) {
            if ($article->delete()) {//true:redirect to index page with success message
                ...
                return redirect('user/helpdesk/kb/article');
    ...
    public function view($slug)
    {
        ...
        $category = new Category();
        $aid = $article->where('id', $slug)->first();
        $description = strip_tags($aid->description);
        $id = $aid->id;
        /* define the selected fields */
        $assign = $relation->where('article_id', $id)->pluck('category_id');
        /* get the attributes of the category */
        ...
        /* send to the edit page */
        try {
            return view('app.user.helpdesk.kb.article.view', compact('assign', 'article', 'category', 'title', 'sub_title', 'base', 'method', 'description'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
    public function show($slug)
    {
        ...
        $description = strip_tags($aid->description);
        try {
            return view('app.user.helpdesk.kb.article.show', compact('assign', 'article', 'category', 'title', 'sub_title', 'base', 'method', 'aid', 'description'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
