...
class TicketsController extends UserAdminController
{
    public function dashboard()
    {
        ...
        return view(
            'app.user.helpdesk.tickets.dashboard',
            compact(
                'title',
                'sub_title',
                'base',
   ...
   /**
    ...
    public function show($id)
    {
           ...
           $ticket_replies = DB::table('ticket_replies')->select('id', 'user_id', 'title', 'body', 'created_at')
            ->where('ticket_id', $id)
   ...
   /**
    ...
    public function destroy($id)
    {
        ...
        $ticket->delete();
        Session::flash('flash_notification', array('level' => 'success', 'message' =>trans('ticket_details.ticket_deleted_successfully')));
        return redirect('user/helpdesk/ticket');
    }
    ...
    public function data(Request $request)
    {
        ...
        try {
            ...
            if ($request->ticketnum <> 'null') {
                ...
                if ($user_id == null) {
                     $tickets = Ticket::where('tickets.ticket_number', 0)
                     ->where('user_id', Auth::user()->id)
                         ->select('tickets.id', 'ticket_number', 'subject', 'status', 'priority', 'department', 'category');
                } else {
                    $tickets = Ticket::where('tickets.ticket_number', $request->ticketnum)
                     ->where('user_id', Auth::user()->id)
                         ->select('tickets.id', 'ticket_number', 'subject', 'status', 'priority', 'department', 'category');
                }
            } else {
