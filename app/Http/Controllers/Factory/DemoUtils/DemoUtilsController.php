...
class DemoUtilsController extends AdminController
{
    ...
    public function dummynetwork($userslimit)
    {
        ...
        for ($i = 0; $i < $limit; $i++) {
            ...
            $data['leg']              = $position;
            $data['shipping_country'] = "Malaysia";
            $data['shipping_state']   = "Johor" ;
    ...
    */
    ...
    public function dummycategory($categorylimit)
    {
        ...
        if (isset($categorylimit)) {
            ...
            $progress = new ProgressBar($output, $limit);
            $progress->start();
            $cat = (['food', 'sports', 'medicine', 'fashion', 'back bags','toys']);
        for ($i = 1; $i <= $limit; $i++) {
            $category = new Category();

            $category->category_name = $cat[$i];
            $category->status = 'yes';
            $category->save();
    ...
    public function dummyproduct($productlimit)
    {
        ...
        if (isset($productlimit)) {
         ...
         // $limit = 5;
             ...
             $pro = (['T-shirt', 'Football', 'Shoes', 'Tablets', 'Toy car','Black Bags','Jeans','Chair','Bottels','Laptop']);
             $category_id = Category::where('status', '=', 'yes')->pluck('id')->toArray();
        for ($i = 0; $i < $limit; $i++) {
            $create_product = new Product();
            $create_product->name = $pro[$i];
            $create_product->description = $faker->realText(10);
            $create_product->quantity = 100;
            $create_product->price = 100 + 2* $i;
            $create_product->status = 'yes';
            $create_product->category_id = $category_id[mt_rand(0, count($category_id) - 1)];
            $image = 'product_'.str_pad($i+1, 2, "0", STR_PAD_LEFT).'.jpg';
            $create_product->image = $image;
            $create_product->save();
