...
class Payout extends Model
{
    ...
    protected $fillable = ['user_id', 'amount','status','payment_mode','payment_type','tx_hash','commission_id'];
    ...
    {
        return self::where('user_id', $user_id)->where('status','released')->sum('amount');
    }
    ...
    public static function confirmPayoutRequest($req_id, $amount)
    {
        return DB::table('payout_request') 
            ->where('id', $req_id)
            ->update(array('status' => 'released','amount'=>$amount, 'payment_mode'=>'manual/bank', 'updated_at'=> date("Y-m-d H:i:s"), 'released_date'=> date("Y-m-d H:i:s")));
    }
    ...
    {   
        $total_payout = self::get();
        $total_amount = $total_payout->sum('amount');
        $total_released_amount = $total_payout->where('status', 'released')->sum('amount');
        $per_amount = 0;
        if ($total_amount > 0) {
