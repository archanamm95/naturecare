    ...
    <thead>
        <tr>
            ...
            <?php foreach ($locales as $locale) : ?>
                ...
                <?php if (!array_key_exists($locale, $displayLocales)) {
                    continue;
                } ?>
                <?php if ($col < 3) : ?>
                    <?php if ($col === 0) : ?>
            ...
            <th width="<?=$mainWidth?>%"><?= $locale ?>&nbsp;
                ...
                -->
                    ...
                    <?php else : ?>
            <th width="<?=$mainWidth?>%"><?= $locale ?></th><?php
                    endif;?>
                <?php else : ?>
            <th><?= $locale ?>
                    <?= ifEditTrans($package . '::messages.auto-translate-disabled') ?>
    ...
    <tbody>
        ...
        foreach ($translations as $key => $translation) {
            ...
            foreach ($locales as $locale) {
                if (!array_key_exists($locale, $displayLocales)) {
                    continue;
                }
                ...
                if (isset($translation[$locale])) {
                    ...
                    if ($trans->status !== 0) {
                        if ($trans->status == 1 || $trans->value != $trans->saved_value) {
                            $has_changed[$locale] = true;
                        } else {
                            $has_changes_cached[$locale] = $trans->value != '' && $trans->status === 2;
                        }
                    }
                }
            }
