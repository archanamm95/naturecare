...
<div id="ticket-dashboard">    
    <div class="panel">
        <div class="panel-body text-center">
            ...
            <p class="mb-15">{{trans('ticket_details.view_and_manage_tickets_submitted')}}</p>
            <a href="#" class="btn bg-warning-400">{{trans('menu.view_statistics')}}</a>
        </div>
    </div>
    <div class="row">
        ...
        <div class="col-sm-6 col-md-3">
            <div class="panel panel-body bg-danger-400 has-bg-image">
                <div class="media no-margin">
                    ...
                    <div class="media-body text-right">
                        <h3 class="no-margin">{{$status}} - 12 {{trans('ticket_details.tickets')}}</h3>
...
<h3>{{trans('ticket_details.category')}}</h3>
        ...
        <div class="col-sm-6">
            <div class="panel panel-flat border-top-info border-bottom-info">
                <div class="panel-heading">
                    <h4 class="panel-title text-semibold color-green">{{trans('ticket_details.categories')}}<a class="heading-elements-toggle"><i class="icon-more"></i></a></h4>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-lg">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>{{trans('ticket_details.name')}}</th>
                                    <th><i class="icon-menu7"></i> {{trans('ticket_details.actions')}}</th>
                                </tr>
                            </thead>
                            <tbody>
                ...
                @endforeach
                    ...
                    <hr/>
                    <form class="form-vertical categoryform" role="form" method="POST" action="{{ URL::to('user/post_ticket_categories') }}">
...
<h3>{{trans('ticket_details.priority')}}</h3>
<form class="form-horizontal" role="form" method="POST" action="{{ URL::to('user/post_ticket_priority') }}">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <div class="form-group">
...
<h3>{{trans('ticket_details.tags')}}</h3>
<form class="form-horizontal" role="form" method="POST" action="{{ URL::to('user/post_ticket_tags') }}">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <div class="col-sm-12">
        <div class="form-group">
...
<h3>{{trans('ticket_details.ticket')}}</h3>
