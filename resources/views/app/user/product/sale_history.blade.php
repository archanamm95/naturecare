        ...
        <tbody>
          @foreach($report as $key=>$item)
           <tr>
             ...
             <td>{{ $item['buyername']}}{{ $item['buyerlastname']}}({{ $item['buyeremail']}})</td>
             <td>{{$item['date']}}</td>
             <td> {{ $item['pay_by'] }}</td>

             <!-- <td>
              <a href="{{{URL::to('user/get-document/'.$item['invoice']) }}}" target="_blank">
                <svg class="feather"><use xlink:href="/backend/icons/feather/feather-sprite.svg#eye"></use></svg></a>
            </td> -->
            <td>

              <a href="{{config('app.site_url')}}/index.php?route=account/order/order_info&order_id={{base64_encode($item['invoice'])}}" target="_blank">
                <svg class="feather"><use xlink:href="/backend/icons/feather/feather-sprite.svg#eye"></use></svg></a>
            </td>
            <td>
