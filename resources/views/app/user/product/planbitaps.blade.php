...
<div class="row">
    <div class="col-lg-5 offset-lg-4">
      ...
      <div class="text-center mb-3">
                ...
                <h5 class="mb-0"> {{trans('products.bitaps_payment')}}</h5>
...
@parent
  ...
  <script type="text/javascript">
     setInterval(function(){
            $.get("{{url('user/get-purcomplete-status/'.$trans_id)}}", function( data ) { 
                 if(data['status'] == 'finish'){
                        window.location.href = 'purchase/complete/'+data['id'];
                 }
                 if(data['status'] == 'complete'){
                            $(".ajax-loader").attr("src","{{ url('global_assets/pp.gif')}}");
