        ...
        <div class="col-sm-6">
            <div class="card card-flat border-top-info border-bottom-info">
                <div class="card-header header-elements-inline">
                    <h4 class="card-title text-semibold color-green">Categories<a class="header-elements-toggle"><i class="icon-more"></i></a></h4>
                ...
                @endforeach
                    ...
                    <form class="form-vertical categoryform" role="form" method="POST" action="{{ URL::to('admin/post_ticket_categories') }}">
                        ...
                        <div class="form-group">
                            <label class="col-form-label">{{trans('ticket_details.add_category')}}</label>
...
<h3>priority</h3>
<form class="form" role="form" method="POST" action="{{ URL::to('admin/post_ticket_priority') }}">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <div class="form-group">
        <label class="col-md-2 col-form-label">{{trans('ticket_details.priority')}}</label>
        <div class="col-md-4">
...
<h3>Tags</h3>
<form class="form" role="form" method="POST" action="{{ URL::to('admin/post_ticket_tags') }}">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <div class="col-sm-12">
        <div class="form-group">
            <label class="col-md-2 col-form-label">{{trans('ticket_details.tags')}}</label>
            <div class="col-md-4">
...
<h3>Ticket</h3>
<form class="form" role="form" method="POST" action="{{ URL::to('admin/save_ticket') }}" enctype="multipart/form-data">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <div class="form-group">
        <label class="col-md-2 col-form-label"> {{trans('ticket.category')}} </label>
        <div class="col-md-8">
    ...
    <div class="form-group">
        <label class="col-md-2 col-form-label"> {{trans('ticket.priority')}} </label>
        <div class="col-md-8">
    ...
    <div class="form-group">
        <label class=" col-md-2 col-form-label" for="tags"> {{trans('ticket.tags')}} </label>
        <div class="col-md-8">
            <select data-placeholder={{trans( 'ticket.choose_tags')}} name="tags[]" id="ticket_tags" class=" col-sm-6 form-control chosen-select" multiple data-parsley-mincheck="0" required data-parsley-trigger="change">
                @foreach($tags as $tags_carousel-item)
                <option {{ (in_array($tags_carousel-item->id,$ticket_tag)) ? 'selected' : '' }} value="{{$tags_carousel-item->id}}">{{$tags_carousel-item->tags}}</option>
                @endforeach
            </select>
            {!! $errors->first('tags', '
            <label class="col-form-label required " for="tags">:message</label>')!!}
        </div>
    </div>
    <div class="form-group">
        <label class="col-md-2 col-form-label"> {{trans('ticket.subject')}} </label>
        <div class="col-md-8">
            <input type="text" class="form-control" name="ticket_subject" required placeholder={{trans( 'ticket.pls_enter_subject')}}>
        </div>
    </div>
    <div class="form-group">
        <label class="col-md-2 col-form-label">{{trans('ticket.description')}}</label>
        <div class="col-md-8">
            <label class="col-form-label">{{ trans('mail.content') }}:</label>
            <div class="m-b-15">
