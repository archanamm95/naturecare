...
@endsection
        ...
        @section('main')
<!-- Single line -->
@if(app('request')->input('create') === 'true')

<div class="col-sm-12">

    <div class="card card-flat">
        <div class="card-header header-elements-inline">
            <h5 class="card-title">{{trans('ticket.create_ticket')}}</h5>
...
<!--             <button type="button" class="btn btn-xs btn-primary text-white" id="filter" data-toggle="collapse" data-target="#filterticketpanel" title="Filter tickets"><i class="icon-filter3"></i>&nbsp;</button> {!! $ticket_priorities->contains('priority',app('request')->input('priority')) ? '
                    ...
                    <form method="GET" action="{{url('admin/helpdesk/ticket')}}" accept-charset="UTF-8" class="form-horizontal" id="filter-form">
                            ...
                            <div class="col-sm-3">
                               {{trans('ticket.priority')}}
                                <select class="select2 filter form-control" name="priority" id="priorities-filter">
                                    <option value="all" selected>{{trans('ticket.all')}}</option>
                                    @foreach($ticket_priorities as $priority)
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-3">
                                ...
                                <input type="text" class="form-control" id="key-word" name="key-word" placeholder="{{trans('ticket.search_member')}}">
                            ...
                            <div class="col-sm-3">
                                ...
                                <label for="overduechooser">{{trans('ticket.show_only_overdue_items')}}</label>
            ...
            <table data-overdue="{{app('request')->input('overdue') ? app('request')->input('overdue') : 'false' }}" data-priority="{{app('request')->input('priority') ? app('request')->input('priority') : 'All' }}" data-department="{{app('request')->input('department') ? app('request')->input('department') : 'All' }}" data-category="{{app('request')->input('category') ? app('request')->input('category') : 'All' }}" data-status="{{app('request')->input('status') ? app('request')->input('status') : 'All' }}" data-deleted="{{app('request')->input('deleted') ? app('request')->input('deleted') : '' }}" data-ticketnumber="{{app('request')->input('ticket-number') ? app('request')->input('ticket-number') : 'null' }}"
            data-username="{{app('request')->input('key_user_hidden') ? app('request')->input('key_user_hidden') : 'null' }}" data-userkeyword="{{app('request')->input('key-word') ? app('request')->input('key-word') : 'null' }}"  class="table datatable-basic table-striped table-hover changestatuswrapper" id="ticket-table" data-search="false" >
                <thead>
                    <tr>
                        <th>
